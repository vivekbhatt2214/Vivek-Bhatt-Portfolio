...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Interview slots are fixed to 30-minute blocks from 9:00 AM through
    // 9:00 PM (the final block starts at 8:30 PM). Never trust a manually
    // crafted client request outside this range.
    if (!isValidInterviewSlot(cleanStartTime, cleanEndTime)) {
      return NextResponse.json(
        { error: "Interview slots are available every 30 minutes from 9:00 AM to 9:00 PM." },
        { status: 400 }
      );
    }
    ...
    // The portfolio owner works in India time. A same-day slot is valid only
    // when its start time is still in the future. This server-side check keeps
    // the rule intact even if a client manually edits the request.
    const indiaNowParts = new Intl.DateTimeFormat("en-GB", {
    ...
    const indiaPart = (type: string) =>
      Number(indiaNowParts.find((part) => part.type === type)?.value || 0);
    const indiaDateKey = `${indiaPart("year")}-${String(indiaPart("month")).padStart(2, "0")}-${String(indiaPart("day")).padStart(2, "0")}`;
    const requestDateKey = new Intl.DateTimeFormat("en-CA", {
      timeZone: "Asia/Kolkata",
      year: "numeric",
      month: "2-digit",
      day: "2-digit",
    ...
    if (requestDateKey === indiaDateKey) {
      const match = cleanStartTime.match(/^(\d{1,2}):(\d{2})/);
      if (match) {
        const slotMinutes = Number(match[1]) * 60 + Number(match[2]);
        const currentMinutes = indiaPart("hour") * 60 + indiaPart("minute");
        if (slotMinutes <= currentMinutes) {
          return NextResponse.json({ error: "This time slot has already passed. Please select a later slot." }, { status: 400 });
        }
      }
    }
...
Interview Booking System
      ...
      await db.query(`UPDATE public.interviewbooking SET source_message_id=$1 WHERE id=$2`, [ownerMailResult.messageId || null, booking.id]);
