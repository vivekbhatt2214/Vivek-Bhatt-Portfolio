...
export function ensurePortfolioSchema() {
  if (!ready) {
    ready = createSchema().catch((error) => {
      // Never cache a failed migration forever. A later request can retry it.
      ready = null;
      throw error;
    });
  }
...
async function createSchema() {
  await db.query(`
    ...
    CREATE TABLE IF NOT EXISTS project_download_requests (
      id BIGSERIAL PRIMARY KEY,
      project_id BIGINT,
    ...
    CREATE INDEX IF NOT EXISTS project_download_email_idx
      ON project_download_requests(email);
   ...
   * Numeric old values are preserved. Anything non-numeric is converted to
   * NULL rather than crashing the migration.
   */
  await db.query(`
    ALTER TABLE project_download_requests
    ...
    ALTER TABLE portfolio_resumes
      ADD COLUMN IF NOT EXISTS file_id BIGINT;
  `);

  // The current application uses numeric file IDs. Some older versions of
  // the portfolio created portfolio_files.id as UUID/text. In that case a
  // normal ALTER ... TYPE BIGINT cannot work, and uploads later return NaN.
  // Migrate the file table itself to a clean BIGINT key while keeping the
  // stored binary data intact.
  await ensurePortfolioFileIdIsBigInt();

  await normalizeNumericColumn("portfolio_projects", "project_file_id");
  await normalizeNumericColumn("portfolio_projects", "image_file_id");
  await normalizeNumericColumn("portfolio_certificates", "image_file_id");
  await normalizeNumericColumn("portfolio_certificates", "file_id");
  await normalizeNumericColumn("portfolio_resumes", "file_id");
...
async function ensurePortfolioFileIdIsBigInt() {
  ...
  if (!current || current === "int8") return;
  ...
  if (["int2", "int4", "numeric"].includes(current)) {
    ...
    // UUID/text IDs cannot be cast to BIGINT. Give every existing file a new
    // numeric ID without touching file_name, mime_type or BYTEA data.
    await db.query(`
      ALTER TABLE portfolio_files
      ...
      WITH numbered AS (
        SELECT
          ctid,
          ROW_NUMBER() OVER (ORDER BY created_at ASC, id::text ASC) AS new_id
        FROM portfolio_files
      )
      UPDATE portfolio_files AS pf
      SET __new_file_id = numbered.new_id
      FROM numbered
      WHERE pf.ctid = numbered.ctid;
    `);

    // Remove any primary-key constraint currently attached to the old ID.
    await db.query(`
      DO $$
      DECLARE
        pk RECORD;
    ...
    await db.query(`
      ALTER TABLE portfolio_files DROP COLUMN id;
      ALTER TABLE portfolio_files RENAME COLUMN __new_file_id TO id;
      ALTER TABLE portfolio_files ALTER COLUMN id SET NOT NULL;
      ALTER TABLE portfolio_files ADD PRIMARY KEY (id);
    `);
  }

  await db.query(`
    CREATE SEQUENCE IF NOT EXISTS portfolio_files_id_seq;
    SELECT setval(
      'portfolio_files_id_seq',
      COALESCE((SELECT MAX(id) FROM portfolio_files), 0),
      COALESCE((SELECT MAX(id) FROM portfolio_files), 0) > 0
    );
    ALTER TABLE portfolio_files
      ALTER COLUMN id SET DEFAULT nextval('portfolio_files_id_seq');
    ALTER SEQUENCE portfolio_files_id_seq
      OWNED BY portfolio_files.id;
  `);
}
...
async function normalizeNumericColumn(
  ...
  if (!current || current.udt_name === "int8") return;

  // A UUID/text identifier cannot safely become a bigint primary key.
  // File uploads in this project are numeric IDs, so leave unusual legacy
  // identifiers alone instead of making the whole schema migration fail.
  const numericTypes = new Set(["int2", "int4", "int8", "numeric"]);
  if (!numericTypes.has(String(current.udt_name))) {
    if (primaryKey) return;
  }

  const qualifiedTable = `"${table.replace(/"/g, '""')}"`;
  const qualifiedColumn = `"${column.replace(/"/g, '""')}"`;
...
async function seedLegacyProjects() {
  ...
  try {
    const result = await db.query(`SELECT * FROM project LIMIT 100`);

    for (const row of result.rows) {
      const title = String(row.title ?? row.name ?? "Project");
      const slug =
        String(
          row.slug ??
            title
      ...
      await db.query(
        ...
        [
          ...
          row.insights ?? null,
          row.outcome ?? null,
          String(row.category ?? "Data Analytics"),
          JSON.stringify(tech),
    ...
    console.warn("Legacy project seed skipped:", error);
  }
}
