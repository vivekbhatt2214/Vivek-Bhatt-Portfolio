...
import { TIME_SLOTS, type TimeSlot } from "@/lib/interview-slots";
...
export default function InterviewBooking() {
  ...
  const isSlotPast = (date: Date, slot: TimeSlot) => {
    const india = getIndiaNow();
    const selectedYear = date.getFullYear();
    const selectedMonth = date.getMonth() + 1;
    const selectedDay = date.getDate();

    if (selectedYear !== india.year || selectedMonth !== india.month || selectedDay !== india.day) {
      return false;
    }

    const [hours, minutes] = slot.id.split(":").map(Number);
    return hours * 60 + minutes <= india.minutes;
  };
          ...
          <div className="booking-slot-panel">
            ...
            <div className="slot-grid">
              ...
              {TIME_SLOTS.map((slot) => {
                ...
                const disabled = !selectedDate || past;
            ...
            {selectedDate && selectedDate.toDateString() === new Date().toDateString() && (
              <div className="slot-info-message">
                Today&apos;s slots before the current India time are unavailable.
              </div>
            )}
